...
class _ShootingStarFieldState extends State<ShootingStarField> with SingleTickerProviderStateMixin {
    ...
    for (int i = 0; i < widget.starCount; i++) {
      _shootingStars.add(
        ShootingStar(
          ...
          length: _random.nextDouble() * 0.3 + 0.1, // Length between 0.1 and 0.4
          angle: _random.nextDouble() * 0.4 + 0.2, // Angle between 0.2 and 0.6 radians
...
class ShootingStarPainter extends CustomPainter {
  ...
  ShootingStarPainter({required this.shootingStars, required this.animation});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var star in shootingStars) {
      ...
      if (cyclePosition < 0.3) {
        final progress = cyclePosition / 0.3; // Normalize to 0-1 within the active phase
